...
export function ClickAttendance({ classes, students, teachers }: { classes: SchoolClass[]; students: Student[]; teachers: Teacher[] }) {
  ...
  return <form action={action} className="click-attendance-form">
    ...
    <section className="optional-area"><div><strong>Keterangan tambahan</strong><small>Opsional dan berlaku untuk semua pilihan</small></div><button type="button" aria-expanded={showNotes} className="button button-secondary small" onClick={() => setShowNotes((value) => !value)}>{showNotes ? "Tutup" : "Tambah keterangan"}</button>{showNotes && <textarea name="notes" aria-label="Keterangan tambahan" rows={2} placeholder="Tulis keterangan bila diperlukan..." />}<label className="checkbox-field"><input name="isConfirmed" type="checkbox" checked={isConfirmed} onChange={(event) => setIsConfirmed(event.target.checked)} /><span><strong>Sudah dikonfirmasi</strong><small>Berlaku untuk semua nama terpilih.</small></span></label></section>
