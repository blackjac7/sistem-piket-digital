...
export default async function ReportsPage() {
  ...
  const summaryEntries = [...classSummary];
  return <><PageHeader title="Rekap dan audit" description="Ringkasan ketidakhadiran serta jejak perubahan sistem." action={<a className="button button-secondary" href="/api/reports/xlsx"><Download /> Unduh Excel</a>} /><section className="report-stats"><article><ClipboardList /><span><strong>{allCount.value}</strong>Total catatan</span></article><article><Activity /><span><strong>{pending.value}</strong>Belum dikonfirmasi</span></article><article><ShieldCheck /><span><strong>{audit.length}</strong>Aktivitas terakhir</span></article></section><section className="dashboard-grid"><article className="panel wide"><div className="panel-header"><div><h2>Ringkasan per kelompok</h2><p>Akumulasi seluruh data</p></div></div><div className="mobile-data-view"><div className="mobile-record-list">{summaryEntries.map(([name, item]) => <article className="mobile-record report-mobile-record" key={name}><div className="mobile-record-heading"><span><strong>{name}</strong><small>{item.total} total catatan</small></span><b>{item.total}</b></div><dl className="mobile-metric-grid"><div><dt>Sakit</dt><dd>{item.sakit}</dd></div><div><dt>Izin</dt><dd>{item.izin}</dd></div><div><dt>Alpa</dt><dd>{item.alpa}</dd></div><div><dt>Dinas</dt><dd>{item.dinas}</dd></div></dl></article>)}{!summaryEntries.length && <p className="empty-state">Belum ada data.</p>}</div></div><div className="table-scroll desktop-data-view"><table><thead><tr><th>Kelompok</th><th>Sakit</th><th>Izin</th><th>Alpa</th><th>Dinas</th><th>Total</th></tr></thead><tbody>{summaryEntries.map(([name, item]) => <tr key={name}><td><strong>{name}</strong></td><td>{item.sakit}</td><td>{item.izin}</td><td>{item.alpa}</td><td>{item.dinas}</td><td><strong>{item.total}</strong></td></tr>)}{!summaryEntries.length && <tr><td colSpan={6} className="empty-state">Belum ada data.</td></tr>}</tbody></table></div></article><article className="panel"><div className="panel-header"><div><h2>Jejak audit</h2><p>20 aktivitas terbaru</p></div></div><div className="audit-list">{audit.map((item) => <div className="audit-item" key={item.id}><span className="audit-dot" /><div><strong>{item.description}</strong><small>{item.user || "Sistem"} · {formatDateTimeId(item.createdAt)}</small></div><StatusPill>{item.action}</StatusPill></div>)}{!audit.length && <p className="empty-state">Belum ada aktivitas.</p>}</div></article></section></>;
}
