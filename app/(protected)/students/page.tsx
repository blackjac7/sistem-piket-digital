...
export default async function StudentsPage({ searchParams }: { searchParams: Promise<{ class?: string }> }) {
  ...
  const classes = await db.select({ id: schoolClasses.id, name: schoolClasses.name, total: sql<number>`count(${students.id})` }).from(schoolClasses).leftJoin(students, eq(schoolClasses.id, students.classId)).groupBy(schoolClasses.id).orderBy(schoolClasses.grade, schoolClasses.name);
  const selectedId = Number(query.class) || classes[0]?.id || 0;
  const selected = classes.find((item) => item.id === selectedId) || classes[0];
  const roster = selected ? await db.select({ name: students.name }).from(students).where(eq(students.classId, selected.id)).orderBy(students.name) : [];
