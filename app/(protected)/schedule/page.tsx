...
export default async function SchedulePage() {
  ...
  return <>
    ...
    <section className="split-layout compact-form"><article className="panel form-panel"><div className="panel-header"><div><h2><CalendarClock /> Tambah jadwal</h2><p>Guru harus berstatus guru piket</p></div></div><ActionForm action={createScheduleAction} submitLabel="Simpan jadwal">
      <label className="field"><span>Guru piket</span><select name="teacherId" required><option value="">Pilih guru</option>{dutyTeachers.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="field"><span>Hari</span><select name="weekday">{Object.entries(weekdayNames).map(([value, name]) => <option value={value} key={value}>{name}</option>)}</select></label><div className="form-grid two"><label className="field"><span>Mulai</span><input type="time" name="startTime" defaultValue="06:30" /></label><label className="field"><span>Selesai</span><input type="time" name="endTime" defaultValue="14:00" /></label></div>
