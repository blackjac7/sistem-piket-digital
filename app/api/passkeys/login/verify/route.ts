...
export async function POST(request: Request) {
  ...
  try {
    ...
    await createSession(user.id); cookieStore.delete(challengeCookieName); cookieStore.delete(flowCookieName);
    return Response.json({ verified: true, redirectTo: destinationForUser(user) });
  } catch (error) { return Response.json({ error: error instanceof Error ? error.message : "Login passkey gagal." }, { status: 400 }); }
}
